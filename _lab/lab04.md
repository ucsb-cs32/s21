...
Police shootings are a very timely issue with a good deal of national dialogue occuring around these incidents and the related civil rights issues for all US civilians.  Each of these incidents is tragic and complex.  We will be working with them to try to understand regional and demographics trends.  As computer scientist, we can use computing to help reveal patterns and information in data and we are working with this data in the spirit of applying technical tools to a socially relevant data set.  The data captured by the Washington Post includes several data feilds relevant to each incident, and the data does have missing fields (as expected in 'real world' data.  Part of working with such data is to gain experience with imperfect data.  For this lab, we will focus on creating a C++ class representation for the following data fields:<br>
